...
void driver_gpio_config_in_intr(uint8_t driver_gpio_num, gpio_mode_e pull_mode, intr_type_e intr_type, void (*isr_handler_ptr)(void *args));

// Funciones para habilitar y deshabilitar interrupciones en GPIO:
void driver_gpio_disable_intr(uint8_t driver_gpio_num);
void driver_gpio_enable_intr(uint8_t driver_gpio_num, uint8_t int_ena_cfg, intr_type_e intr_type);
...
void driver_enable_cpu_interrupt(uint8_t level);
